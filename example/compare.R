...
ggsave(plot = p, filename = "GATKvsDV_metric_distribution.png", device="png", height = 7, width = 10)

sum_df <- df %>% group_by(cohort) %>% skim(HQ_HOM:INCONSISTENT_AB_HET_RATE)
write.table(sum_df, file = "GATKvsDV_metric_distribution.tsv", sep="\t", row.names=F, quote=F)
